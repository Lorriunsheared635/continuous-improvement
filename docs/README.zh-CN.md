...
- `/continuous-improvement`、`/planning-with-files`、`/discipline`、`/dashboard` 命令
- 自动升级的本能直觉系统
...
包含入门模式的所有功能，额外增加：
- **MCP 服务器**，提供 12 个工具（本能管理、规划文件、导入/导出、观察查看器、仪表盘、本能包）
...
| `ci_plan_init` | 在项目根目录创建 `task_plan.md`、`findings.md`、`progress.md`（专家模式） |
| `ci_plan_status` | 汇总三个规划文件的当前状态（专家模式） |
...
│   └── expert.json                    # 插件清单：12 个工具
├── commands/continuous-improvement.md # /continuous-improvement 命令
├── commands/planning-with-files.md    # /planning-with-files 命令
├── templates/planning-with-files/     # 规划文件模板
...
~/.claude/commands/planning-with-files.md            # 文件化规划命令
~/.claude/instincts/
├── observe.sh                                       # Hook 脚本
├── global/                                          # 全局本能 (*.yaml)
└── <project-hash>/
...
显式初始化文件化规划后，还会在项目根目录创建：
```
task_plan.md                                         # 阶段、状态、决策、错误
findings.md                                          # 研究笔记和来源
progress.md                                          # 会话日志和验证记录
```
...
- [x] Node 测试套件（安装器、hook、MCP 服务器、插件配置、SKILL.md 验证）
...
- [x] **MCP 服务器** — 12 个工具（入门：3 个，专家：额外 9 个），零依赖
