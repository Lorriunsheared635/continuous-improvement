...
function toInstinct(value: unknown): Instinct | null {
...
const DEFAULT_PLANNING_PHASES = ["Research", "Plan", "Execute", "Verify", "Reflect"] as const;
...
function parseYamlInstinct(content: string): Instinct | null {
  const lines = content.split("\n");
  const meta: Partial<Instinct> = {};
  let body = "";
  let inBody = false;
  ...
  for (const line of lines) {
    ...
    const value = rawValue !== "" && !Number.isNaN(Number(rawValue))
      ? Number(rawValue)
      : rawValue;
    (meta as Record<string, string | number | undefined>)[match[1]] = value;
  }
...
function countObservations(projectHash: string): number {
...
function detectLevel(projectHash: string): Level {
...
function writeInstinct(projectHash: string, instinct: Instinct): void {
...
function updateInstinctConfidence(projectHash: string, instinctId: string, delta: number): Instinct | null {
  const instincts = readInstincts(projectHash);
...
function readPlanningTemplate(fileName: string, fallback: string): string {
...
function normalizePhases(value: unknown): string[] {
  ...
  const phases = value
    ...
    .filter((phase): phase is string => phase.length > 0);

  return phases.length > 0 ? phases : [...DEFAULT_PLANNING_PHASES];
}

function formatPhaseChecklist(phases: string[]): string {
  return phases.map((phase) => `- [ ] ${phase}`).join("\n");
}

function renderTaskPlan(goal: string, phases: string[]): string {
...
function writePlanningFile(filePath: string, content: string, force: boolean): "created" | "overwritten" | "preserved" {
...
function parseTaskPlan(content: string): { checkedPhases: string[]; status: string; uncheckedPhases: string[] } {
  const checkedPhases: string[] = [];
  const uncheckedPhases: string[] = [];
...
function hasMeaningfulContent(content: string, placeholders: string[]): boolean {
  return content
    .split(/\r?\n/)
    .map((line) => line.trim())
...
function error(value: string): ToolResult {
  return { content: [{ type: "text", text: value }], isError: true };
}

function handleTool(name: string, params: Record<string, unknown>): ToolResult {
  ...
  switch (name) {
    ...
    case "ci_plan_init": {
      ...
      const taskPlanStatus = writePlanningFile(
        join(workspaceRoot, PLANNING_FILES.taskPlan),
        renderTaskPlan(goal, phases),
        force
      );
      const findingsStatus = writePlanningFile(
        join(workspaceRoot, PLANNING_FILES.findings),
        renderFindings(),
        force
      );
      const progressStatus = writePlanningFile(
        join(workspaceRoot, PLANNING_FILES.progress),
        renderProgress(),
        force
      );
    ...
    case "ci_plan_status": {
      ...
      if (!taskPlanExists && !findingsExists && !progressExists) {
        return text(
          `No planning files found in ${workspaceRoot}. Run ci_plan_init or /planning-with-files to create ${PLANNING_FILES.taskPlan}, ${PLANNING_FILES.findings}, and ${PLANNING_FILES.progress}.`
        );
      }
      ...
      const summary = taskPlanExists
        ...
        : { status: "No task plan yet", checkedPhases: [] as string[], uncheckedPhases: [] as string[] };
    ...
    case "ci_load_pack": {
      ...
      const packInstincts = JSON.parse(readFileSync(packPath, "utf8")) as unknown[];
      const existing = readInstincts(project.hash);
      const existingIds = new Set(existing.map((instinct) => instinct.id));
      let loaded = 0;
